...
import { ThumbsUp, ThumbsDown, Lightbulb, Brain, RefreshCw, Loader2, Sparkles, TrendingUp, TrendingDown, MessageSquare, Star, Target } from 'lucide-react';
...
export default function SMRetrospective() {
    ...
    useEffect(() => {
        leadAPI.retrospective().then(data => {
            ...
            const retro = data[0];
            if (retro?.ai_summary) setAiSummary(retro.ai_summary);
    ...
    const handleGenerate = async () => {
        setLoading(true);
        try {
            const res = await leadAPI.generateRetrospective();
            if (res?.ok) {
