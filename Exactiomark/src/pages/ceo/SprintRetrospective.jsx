...
import { ThumbsUp, ThumbsDown, Lightbulb, Brain, Star, MessageSquare, RefreshCw, Loader2, Sparkles } from 'lucide-react';
...
export default function SprintRetrospective() {
    ...
    const [behaviourFeedback, setBehaviourFeedback] = useState([]);
    ...
    useEffect(() => {
        ceoAPI.improvements().then(data => {
            if (!Array.isArray(data)) return;
            const well = data.filter(i => i.category === 'went_well').map(i => i.text || i.action);
            const didnt = data.filter(i => i.category === 'didnt_go_well').map(i => i.text || i.action);
        ...
        ceoAPI.behaviourFeedback().then(data => {
            if (!Array.isArray(data)) return;
            if (data.length) setAiSummary(data[0]?.aiSummary || '');
            setBehaviourFeedback(data);
        }).catch(() => { });
    }, []);
    ...
    const handleGenerate = async () => {
        setLoading(true);
        try {
            const res = await ceoAPI.generateRetrospective();
            if (res?.ok) {
    ...
    return (
        <div>
            ...
            {/* Behaviour Feedback */}
            {behaviourFeedback.length > 0 && behaviourFeedback.some(m => m.communication > 0) && (
                <AnimatedCard delay={0.42} style={{ marginTop: 20 }}>
                    ...
                    <div className="card-body">
                        <div style={{ display: 'grid', gap: 16 }}>
                            {behaviourFeedback.filter(m => m.communication > 0).map((m, i) => (
